...
export default async function handler(
  ...
  switch (method) {
    case 'GET':
      try {
        const companyWaitlist = await CompanyWaitlist.find(
          {}
    ...
    case 'POST':
      try {
        const companyWaitlist = await CompanyWaitlist.create(
          req.body
        ); /* create a new model in the database */
        res.status(201).json({ success: true, data: companyWaitlist });
      } catch (error) {
        res.status(400).json({ success: false });
      }
      break;
